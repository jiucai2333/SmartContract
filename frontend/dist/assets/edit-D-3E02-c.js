const e=`\uFEFFif (!initAppShell('edit', '在线编辑', '支持合同内容编辑、模板解析、草稿保存与 Word 下载')) throw new Error('auth required');
...
function isBlankUnderlineElement(node) {
    if (node?.nodeType !== Node.ELEMENT_NODE) return false;
    const style = node.getAttribute('style') || '';
    const isUnderline = node.tagName === 'U'
        || node.classList.contains('contract-fill-underline')
        || /text-decoration\\s*:\\s*[^;]*underline/i.test(style)
        || /border-bottom\\s*:\\s*(?!none)/i.test(style);
    if (!isUnderline) return false;
    return !normalizeFieldText(node.textContent).replace(/[.\\u00b7。．]/g, '');
}
...
function renderContractFields(result) {
    ...
    $('#fieldAnalysisSummary').textContent =
        \`基础必填缺失 \${result.requiredMissingCount || 0} 项，风险修改点 \${draftState.analyzedRisks.length} 项\`;
...
function renderContractRisks() {
    ...
    return \`<section class="contract-field-section">
        ...
        \${draftState.analyzedRisks.map((risk, index) => {
            ...
            const replacement = risk.replacement || suggestion;
            return \`<div class="contract-field-card contract-risk-card \${escapeHtml(level)}" data-risk-card="\${index}">
                ...
                <textarea data-risk-input="\${index}" placeholder="修改后条款（可直接回填）">\${escapeHtml(replacement)}</textarea>
                <p class="contract-field-source"><strong>风险原因：</strong>\${escapeHtml(risk.reason || '风险报告未返回详细原因')}</p>
                <p class="contract-field-suggestion"><strong>修改建议：</strong>\${escapeHtml(suggestion || '请法务复核该风险项')}</p>
...
function renderFieldPanel() {
    const fieldHtml = draftState.analyzedFields.map(field => {
        ...
        return \`<div class="contract-field-card">
            ...
            <div class="risk-card-actions">
                <button type="button" class="secondary" data-fill-field="\${escapeHtml(field.fieldKey)}">回填</button>
            </div>
        </div>\`;
...
function createUnderlineFill(value, token) {
    ...
    underline.className = 'contract-fill-underline';
    underline.textContent = value;
    underline.style.display = 'inline-block';
    underline.style.minWidth = \`\${placeholderWidthEm(token)}em\`;
    underline.style.padding = '0 0.35em';
    underline.style.textAlign = 'center';
    underline.style.lineHeight = '1.2';
    return underline;
}
...
function collectUnderlineTargets(root) {
    ...
    const visit = node => {
        ...
        if (isBlankUnderlineElement(node)) {
            targets.push({type: 'element', element: node, token: node.textContent || '____'});
            return;
        }
...
function fillBySourceText(html, field, value) {
    ...
    const candidates = [...container.querySelectorAll('tr, p, li, div')]
        .filter(element => hasFillTarget(element))
        .filter(element => {
            const text = normalizeFieldText(element.textContent);
            return fieldSourceMatchesElement(field, sourceText, text);
        })
        .filter(element => ![...element.children].some(child => {
            const childText = normalizeFieldText(child.textContent);
            return childText && hasFillTarget(child)
                && fieldSourceMatchesElement(field, sourceText, childText);
        }));
...
function hasFillTarget(element) {
    return underlineTokenRegex().test(element.textContent || '') || countUnderlineTokens(element) > 0;
}

function stripUnderlineTokens(value) {
    return normalizeFieldText(value).replace(underlineTokenGlobalRegex(), '');
}

function fieldSourceMatchesElement(field, sourceText, elementText) {
    if (!sourceText || !elementText) return false;
    if (sourceText.includes(elementText) || elementText.includes(sourceText)) return true;

    const sourceCore = stripUnderlineTokens(sourceText);
    const elementCore = stripUnderlineTokens(elementText);
    if (sourceCore && elementCore && (sourceCore.includes(elementCore) || elementCore.includes(sourceCore))) return true;

    const fieldName = normalizeFieldText(field.fieldName);
    const anchorBefore = stripUnderlineTokens(field.anchorBefore);
    const anchorAfter = stripUnderlineTokens(field.anchorAfter);
    const placeholder = normalizeFieldText(field.placeholderText);
    const hasSamePlaceholder = placeholder && (sourceText.includes(placeholder) || elementText.includes(placeholder));
    const hasFieldName = fieldName && (sourceText.includes(fieldName) || elementText.includes(fieldName));
    const hasAnchor = (anchorBefore && elementText.includes(anchorBefore)) || (anchorAfter && elementText.includes(anchorAfter));
    return Boolean((hasSamePlaceholder && (hasFieldName || hasAnchor)) || (hasFieldName && hasAnchor));
}
...
function fillByDocumentStructure(html, field, value) {
    ...
    if (target.tagName === 'TR') {
        ...
        if (labelIndex >= 0) {
            ...
            if (emptyCells.length === 1) {
                emptyCells[0].replaceChildren(createUnderlineFill(value, field.placeholderText || value));
                return {html: container.innerHTML, replaced: true};
            }
        }
    }
    ...
    if (labelPattern.test(text.trim())) {
        const label = text.replace(labelPattern, '$1');
        target.replaceChildren(document.createTextNode(label), createUnderlineFill(value, field.placeholderText || value));
        return {html: container.innerHTML, replaced: true};
    }
    return {html, replaced: false, reason: '未找到可安全替换的位置'};
}

function fillSingleField(fieldKey) {
    const field = draftState.analyzedFields.find(f => f.fieldKey === fieldKey);
    if (!field) return toast('未找到该字段');
    const value = fieldInputValue(fieldKey);
    if (!value) return toast('请先填写字段值');
    let html = draftContent();
    const result = tryFillField(html, field, value);
    if (result.replaced) {
        settingEditorContent = true;
        ...
        updateCharCount();
        toast(\`已回填「\${field.fieldName}」\`);
    } else {
        toast(result.reason || '未找到匹配位置');
    }
}

function tryFillField(html, field, value) {
    let result = fillByLocator(html, field, value, field.placeholderIndex);
...
function locateRiskText(risk) {
    const clause = riskSourceText(risk);
    if (!normalizeRiskClause(clause)) {
        toast('风险报告中没有可定位的原文片段');
        return false;
    }
    const editorRoot = $('#draftEditor [data-slate-editor]') || $('#draftEditor');
    let target = findRiskBlock(editorRoot, risk);
    if (!target) {
...
function activateRiskCard(index, {focusInput = false} = {}) {
    document.querySelectorAll('.contract-risk-card.active').forEach(card => card.classList.remove('active'));
    const card = document.querySelector(\`[data-risk-card="\${index}"]\`);
    if (!card) return;
    card.classList.add('active');
    card.scrollIntoView({behavior: 'smooth', block: 'nearest'});
    if (focusInput) {
        const input = card.querySelector(\`[data-risk-input="\${index}"]\`);
...
function replaceRiskTextInHtml(html, risk, value) {
    ...
    if (matches.length !== 1) {
        const block = findRiskBlock(container, risk);
        if (!block) {
            return {
                html,
                replaced: false,
        ...
        const replacedText = replaceRiskTextInPlainText(blockText, risk, value);
        if (!replacedText.replaced) {
            return {html, replaced: false, reason: replacedText.reason || '已定位疑似条款，但无法可靠替换'};
        }
        block.textContent = replacedText.text;
        return {html: container.innerHTML, replaced: true};
    }
    const match = matches[0];
    match.node.nodeValue = match.node.nodeValue.slice(0, match.offset)
...
function riskSourceText(risk) {
    return String(risk?.clause || risk?.clauseRef || '').trim();
}

function riskArticleRef(risk) {
    const match = riskSourceText(risk).match(/第[一二三四五六七八九十百千万0-9]+条/);
    return match ? match[0] : '';
}

function riskKeywords(risk) {
    const category = String(risk?.category || risk?.riskType || '').toUpperCase();
    const words = new Set();
    const seed = {
        SUBJECT_INFO: ['甲方', '乙方', '主体', '名称', '统一社会信用代码', '授权代表', '地址', '联系人', '联系电话'],
        PAYMENT: ['付款', '支付', '金额', '价款', '发票', '结算', '款项', '账户'],
        TERM: ['期限', '日期', '交付', '验收', '生效', '到期', '解除'],
        DISPUTE_RESOLUTION: ['争议', '管辖', '仲裁', '法院', '诉讼', '法律'],
        LIABILITY: ['违约', '赔偿', '责任', '违约金', '免责', '损失']
    };
    (seed[category] || seed.LIABILITY).forEach(word => words.add(word));
    [riskSourceText(risk), risk?.reason, risk?.suggestion].forEach(value => {
        String(value || '').match(/[\\u4e00-\\u9fa5A-Za-z0-9]{2,12}/g)?.forEach(word => {
            if (!['风险', '原因', '建议', '修改', '合同', '条款', '明确', '具体'].includes(word)) words.add(word);
        });
    });
    return Array.from(words).slice(0, 18);
}

function scoreRiskText(text, risk) {
    const compact = normalizeRiskClause(text);
    if (!compact) return 0;
    const source = normalizeRiskClause(riskSourceText(risk));
    let score = source && compact.includes(source) ? 30 : 0;
    const article = normalizeRiskClause(riskArticleRef(risk));
    if (article && compact.includes(article)) score += 8;
    riskKeywords(risk).forEach(keyword => {
        if (compact.includes(normalizeRiskClause(keyword))) score += keyword.length >= 4 ? 3 : 2;
    });
    return score;
}

function findRiskBlock(root, risk) {
    const blocks = Array.from(root.querySelectorAll('p, div, h1, h2, h3, td, li'))
        .filter(block => (block.textContent || '').trim().length > 0);
    const source = normalizeRiskClause(riskSourceText(risk));
    let target = blocks.find(block => source && normalizeRiskClause(block.textContent).includes(source));
    if (target) return target;
    let best = null;
    let bestScore = 0;
    blocks.forEach(block => {
        const score = scoreRiskText(block.textContent, risk);
        if (score > bestScore) {
            bestScore = score;
            best = block;
        }
...
function replaceRiskTextInPlainText(text, risk, value) {
    const direct = replaceNormalizedSegment(text, riskSourceText(risk), value);
    if (direct.replaced) return direct;
    const sentences = text.match(/[^。；;\\n]+[。；;]?/g) || [text];
    let best = '';
    let bestScore = 0;
    sentences.forEach(sentence => {
        const score = scoreRiskText(sentence, risk);
        if (score > bestScore) {
            bestScore = score;
            best = sentence;
        }
    ...
    if (best && bestScore >= 4) {
        return replaceNormalizedSegment(text, best, value);
    }
    if (scoreRiskText(text, risk) >= 6 && text.length <= 600) {
        return {text: value, replaced: true};
    }
    return {text, replaced: false, reason: '未找到可安全替换的风险原文片段'};
}

function replaceNormalizedSegment(text, source, value) {
    const raw = String(text || '');
    const normalizedSource = normalizeRiskClause(source);
    if (!raw || !normalizedSource) return {text: raw, replaced: false};
    const map = [];
    let normalized = '';
    for (let i = 0; i < raw.length; i++) {
        const ch = normalizeRiskClause(raw[i]);
        if (!ch) continue;
        normalized += ch;
        map.push(i);
    }
    const index = normalized.indexOf(normalizedSource);
    if (index < 0) return {text: raw, replaced: false};
    const start = map[index];
    const end = map[index + normalizedSource.length - 1] + 1;
    return {text: raw.slice(0, start) + value + raw.slice(end), replaced: true};
}
...
async function openExistingDraft(contractId) {
    if (!contractId) return;
    if (hasEditorContent() && !await confirmDialog('当前内容将被替换，是否继续？', {title: '替换确认', confirmText: '替换', type: 'warn'})) return;
    draftState.contractId = Number(contractId);
    draftState.templateId = null;
    draftState.attachmentId = null;
    draftState.latestRiskReport = null;
    draftState.analyzedFields = [];
    draftState.analyzedRisks = [];
    ...
    history.replaceState(null, '', \`/html/edit.html?contractId=\${encodeURIComponent(draftState.contractId)}\`);
    updateStatus('正在加载现有草稿...');
...
async function loadDraftOptions() {
    try {
        ...
        const drafts = (Array.isArray(contracts) ? contracts : [])
            .filter(contract => String(contract.status || '').toUpperCase() === 'DRAFT');
        drafts.forEach(contract => {
            const opt = document.createElement('option');
            opt.value = contract.contractId;
            const title = contract.title || \`合同 #\${contract.contractId}\`;
            const no = contract.contractNo ? \`\${contract.contractNo} · \` : '';
            const counterparty = contract.counterparty ? \` · \${contract.counterparty}\` : '';
            opt.textContent = \`\${no}\${title}\${counterparty}\`;
            select.appendChild(opt);
        });
        if (!drafts.length) {
            const opt = document.createElement('option');
            opt.value = '';
            opt.textContent = '暂无草稿状态合同';
            opt.disabled = true;
            select.appendChild(opt);
        }
        if (draftState.contractId) {
            select.value = String(draftState.contractId);
            $('#useDraftBtn').disabled = false;
        }
    } catch (e) { /* 静默失败，不影响上传和模板流程 */ }
}
$('#draftSelect').addEventListener('change', event => {
    $('#useDraftBtn').disabled = !event.target.value;
});
$('#useDraftBtn').addEventListener('click', () => {
    const contractId = Number($('#draftSelect').value);
    openExistingDraft(contractId).catch(error => toast(error.message));
});
...
$('#contractFieldList').addEventListener('click', event => {
    ...
    if (riskSuggestion) {
        ...
        if (input && draftState.analyzedRisks[index]) {
            const risk = draftState.analyzedRisks[index];
            input.value = risk.replacement || risk.suggestion || '';
            activateRiskCard(index, {focusInput: true});
        }
        return;
    }
    const locateButton = event.target.closest('[data-locate-risk]');
    if (locateButton) {
        const index = Number(locateButton.dataset.locateRisk);
        activateRiskCard(index);
        locateRiskText(draftState.analyzedRisks[index]);
        return;
    }
    const applyButton = event.target.closest('[data-apply-risk]');
    if (applyButton) {
        const index = Number(applyButton.dataset.applyRisk);
        activateRiskCard(index);
        applyRiskSuggestion(index);
    }
    const fillFieldButton = event.target.closest('[data-fill-field]');
    if (fillFieldButton) fillSingleField(fillFieldButton.dataset.fillField);
});
