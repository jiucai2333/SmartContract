...
public class TemplateService {
    ...
    public List<TemplateVO> listVos(String type, String keyword) {
        List<ContractTemplate> templates = list(type, keyword);
        List<Long> fileIds = templates.stream()
                .map(ContractTemplate::getFileId)
        ...
        return templates.stream()
                .map(template -> toVo(template, files.get(template.getFileId())))
                .toList();
    }
    ...
    public TemplateVO toVo(ContractTemplate template) {
        FileInfo file = template.getFileId() == null ? null : fileInfoMapper.selectById(template.getFileId());
        return toVo(template, file);
    }

    private TemplateVO toVo(ContractTemplate template, FileInfo file) {
