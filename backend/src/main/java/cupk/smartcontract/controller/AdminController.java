...
public class AdminController {
    ...
    @AuditOperation(operation = "ADMIN_UPDATE_USER_STATUS", targetType = "USER",
            targetIdParameter = "userId")
    public Map<String, Object> updateStatus(@PathVariable Long userId,
                                            @RequestBody Map<String, Object> body) {
        Integer status = body.get("status") instanceof Number n ? n.intValue() : null;
        if (status == null || (status != 0 && status != 1)) {
            throw new IllegalArgumentException("status 必须为 0（禁用）或 1（启用）");
        }
