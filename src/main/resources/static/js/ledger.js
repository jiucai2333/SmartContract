...
async function openDetailModal(contractId, title) {
    ...
    try {
        ...
        b.innerHTML =
            ...
            '<div class="detail-section"><h4>历史版本 (' + versions.length + ')</h4>' + (versions.length === 0 ? '<p class="hint">暂无版本记录</p>' : versions.map(function (v) { return '<div class="record-item"><span><strong>' + escapeHtml(v.versionNo) + '</strong>' + (lockedIds.has(v.versionId) ? ' <span class="tag tag-green">已锁定</span>' : '') + '</span><span>创建时间：' + escapeHtml(v.createdAt || '-') + '</span><span>创建人：' + escapeHtml(v.createdBy || '-') + '</span></div>'; }).join('')) + '<p class="hint" style="margin-top:8px"><a href="/html/draft.html?contractId=' + contractId + '">查看或编辑合同正文</a></p></div>' +
...
$('#contractRows').addEventListener('click', async function (e) {
    ...
    if (submitBtn) { try { await api('/api/contracts/' + submitBtn.dataset.submit + '/submit', { method: 'POST' }); toast('已提交审批'); await loadContracts(); } catch (err) { toast(err.message); } return; }
