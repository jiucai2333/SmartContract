...
public class FadadaSignatureProvider implements SignatureProvider {
    ...
    public FadadaSignatureProvider(SignatureProperties props, FileInfoMapper fileInfoMapper) {
        this.props = props;
    ...
    private synchronized String getAccessToken() {
        if (cachedToken != null && System.currentTimeMillis() < tokenExpireAt - 60_000) return cachedToken;
        try {
            ...
            if (resp.getBody() != null) {
                ...
                if (raw != null && raw.get("data") instanceof Map) {
                    ...
                    log.info("accessToken 获取成功");
                    return cachedToken;
                }
                log.error("Token响应异常: {}", resp.getBody());
            }
            throw new RuntimeException("Token获取失败");
        } catch (Exception e) {
            log.error("Token异常", e);
            throw new RuntimeException("法大大认证失败", e);
        }
    }
    ...
    private String uploadFile(String token, Long localFileId) {
        try {
            ...
            byte[] bytes = readLocalFile(localFileId);
            ...
            Map<String, Object> r2 = call(token, "/file/process", Map.of("fddFileUrlList", List.of(
                    Map.of("fileType", "doc", "fddFileUrl", fddFileUrl,
                            "fileName", "contract-" + localFileId + ".pdf"))));
    ...
    private byte[] readLocalFile(Long fileId) {
        try {
            var info = fileInfoMapper.selectById(fileId);
            if (info == null) return placeholder();
            var p = java.nio.file.Path.of(System.getProperty("user.dir"), "data", "uploads", info.getObjectKey());
            if (java.nio.file.Files.exists(p)) return java.nio.file.Files.readAllBytes(p);
        } catch (Exception e) { log.error("读文件失败", e); }
        return placeholder();
    }

    private byte[] placeholder() { return "%PDF-1.4".getBytes(StandardCharsets.ISO_8859_1); }
