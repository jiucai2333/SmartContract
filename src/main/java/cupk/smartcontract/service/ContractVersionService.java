...
public class ContractVersionService {
    ...
    public ContractVersion create(Long contractId, String content, String contentHash,
        ...
        String max = findMaxVersionNo(contractId);
        String nextVersionNo;
        if ("SAVE_AS".equals(saveType)) {
            nextVersionNo = "v" + (parseMajorVersion(max) + 1) + ".0";
        } else {
            nextVersionNo = "v" + parseMajorVersion(max) + "." + (parseMinorVersion(max) + 1);
        }
    ...
    private String findMaxVersionNo(Long contractId) {
        List<ContractVersion> list = mapper.selectList(
                new LambdaQueryWrapper<ContractVersion>()
                        .eq(ContractVersion::getContractId, contractId)
                        .orderByDesc(ContractVersion::getCreatedAt)
                        .last("LIMIT 1"));
        if (!list.isEmpty()) return list.get(0).getVersionNo();
        return "v0.0";
    }

    private int parseMajorVersion(String versionNo) {
        try {
            if (versionNo != null && versionNo.startsWith("v")) {
                return Integer.parseInt(versionNo.substring(1).split("\\.")[0]);
            }
    ...
    private int parseMinorVersion(String versionNo) {
        try {
            if (versionNo != null && versionNo.startsWith("v")) {
                String[] parts = versionNo.substring(1).split("\\.");
