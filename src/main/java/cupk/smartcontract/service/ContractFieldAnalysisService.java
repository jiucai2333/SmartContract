...
public class ContractFieldAnalysisService {
    ...
    public ContractFieldAnalysisVO analyze(ContractFieldAnalysisRequest request) {
        ...
        try {
            ...
            int beforeAiMerge = fields.size();
            MergeStats mergeStats = mergeAiFields(fields, aiFields, text);
            mode = "QWEN_AI_ENHANCED";
            log.info("[field-analysis] 合并后接受字段数={}, 最终字段数={}, analysisMode={}",
                    mergeStats.accepted(), fields.size(), mode);
            ...
            notice = "已基于当前合同正文识别待填写字段并生成可用建议值。";
        } catch (Exception ex) {
            log.warn("[field-analysis] 千问字段识别失败，降级为规则识别: {}: {}",
                    ex.getClass().getSimpleName(), ex.getMessage());
            notice = "AI 服务不可用，已降级为规则识别：" + clip(ex.getMessage() == null ? "未知错误" : ex.getMessage(), 120);
        }
        return response(fields, mode, notice);
    }

    public List<String> findApprovalBlockingMissing(String html, String contractType) {
        ContractFieldAnalysisVO result = analyze(new ContractFieldAnalysisRequest(
                null, null, html, null, contractType));
    ...
    private Map<String, ContractField> htmlPlaceholderFields(String html) {
        ...
        Elements blocks = Jsoup.parseBodyFragment(html).body().select(BLOCK_SELECTOR);
        String role = "";
        int blockIndex = 0;
